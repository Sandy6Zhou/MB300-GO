 ...
 * */
static void audio_effect_dev3_run(struct effect_dev3_node_hdl *hdl, s16 *data, u32 data_len)
{
    //do something
    ...
    while (1) {
        ...
        if (pack_frame_num) {
            ...
            while (pack_frame_num--) {
                audio_effect_dev3_run(hdl, (s16 *)((int)hdl->pack_frame->data + offset), hdl->frame_len);
...
#else
        audio_effect_dev3_run(hdl, (s16 *)frame->data, frame->len);
...
static void effect_dev3_ioc_start(struct effect_dev3_node_hdl *hdl)
{
    ...
    printf("effect_dev3_ioc_start, sr: %d, ch: %d, bitw: %d", hdl->sample_rate, hdl->ch_num, hdl->bit_width);
#if EFFECT_DEV3_FRAME_POINTS
    hdl->frame_len = EFFECT_DEV3_FRAME_POINTS * hdl->ch_num * (2 << hdl->bit_width);
