...
#define DC_REG_SWITCH_BIT_POWER      0      /* 开机开关 Bit0 */
...
#define DC_REG_SWITCH_BIT_FREQ       12     /* 频率 50/60Hz */
#define DC_REG_SWITCH_BIT_BT_AUDIO   14     /* 蓝牙音箱工作状态：1=播放中 0=未工作 */
#define DC_REG_SWITCH_BIT_BT_OTA     15     /* 蓝牙固件升级：1=升级中 0=未升级 */
...
static uint8 s_dc_boot_ota_bit_cleared = 0;            /* 上电后已写 Bit15=0 */
static uint8 s_dc_pending_ota_bit_off = 0;             /* OTA OFF 时基线未就绪，延后清 Bit15 */
...
/* DC 关机检测：0=通讯离线  1=0x0001 Bit0；切换时只改此宏与查询函数 */
...
static uint8 dc_modbus_led_to_ff01_sw(uint8 mode)
{
    ...
    switch (mode)
    {
        ...
        case DC_LED_MODE_20PCT:
            return 0x04; /* 100: 不闪+20% */
        case DC_LED_MODE_50PCT:
            return 0x02; /* 010: 不闪+50% */
 ...
 *   仅在线阶段累加 g_dc_timeout_count（封顶 DC_OFFLINE_CONSECUTIVE_COUNT），
 *   达阈值置 g_dc_online=0；已离线后不再计数/打日志
...
/* 返回 1：已发起 0x0001 指定位写请求；0：未发送（忙/基线未就绪/无需变化） */
static int dc_send_switch_bit_once(uint8 bit, uint8 on)
{
    uint16 cur = 0;
    uint16 target = 0;
...
/* 0x0001 基线首次就绪：上电清 Bit15，并处理 OTA OFF 挂起的清位 */
static void dc_on_switch_reg_ready(void)
{
    uint16 cur = 0;
...
/* 进入运输模式：Modbus 05 写 0x0001 Bit10 */
int my_dc_enter_transport_mode(void)
{
    ...
    // 设置控制请求
    g_dc_ctrl_req.valid = 1;
    g_dc_ctrl_req.sw_id = MY_DC_SW_AC; // 占位，实际下发 target_reg_value；勿走 with_ack 重试
